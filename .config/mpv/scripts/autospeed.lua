...
function main(name, fps)
	...
	if _global.options["speed"] == true then
		determineSpeed()
		if
			_global.temp["speed"] >= _global.options["minspeed"]
			and _global.temp["speed"] <= _global.options["maxspeed"]
		then
...
function setOSD()
	_global.temp["output"] = (
		_global.osd_start
		.. "{\\b1}Original monitor refresh rate{\\b0}\\h\\h"
		.. _global.temp["start_drr"]
		.. "Hz\\N"
		.. "{\\b1}Current  monitor refresh rate{\\b0}\\h\\h"
		.. _global.temp["drr"]
		.. "Hz\\N"
		.. "{\\b1}Original video fps{\\b0}\\h\\h\\h\\h\\h\\h\\h\\h\\h\\h\\h\\h\\h\\h\\h\\h\\h\\h\\h\\h\\h"
		.. _global.temp["fps"]
		.. "fps\\N"
		.. "{\\b1}Current  video fps{\\b0}\\h\\h\\h\\h\\h\\h\\h\\h\\h\\h\\h\\h\\h\\h\\h\\h\\h\\h\\h\\h\\h"
		.. (_global.temp["fps"] * _global.temp["speed"])
		.. "fps\\N"
		.. "{\\b1}Original mpv speed setting{\\b0}\\h\\h\\h\\h\\h\\h"
		.. _global.confSpeed
		.. "x\\N"
		.. "{\\b1}Current  mpv speed setting{\\b0}\\h\\h\\h\\h\\h\\h"
		.. _global.temp["speed"]
		.. "x"
		.. _global.osd_end
	)
...
function setXrandrRate(mode)
	...
	local vars = {
		vid = nil,
		time_pos = nil,
		vdpau = (mp.get_property("options/vo") == "vdpau" or mp.get_property("options/hwdec") == "vdpau"),
	}
	...
	_global.utils.subprocess({
		["cancellable"] = false,
		["args"] = { [1] = "xrandr", [2] = "--output", [3] = _global.options["display"], [4] = "--mode", [5] = mode },
	})
	if _global.options["spause"] then
		if tonumber(_global.options["spause"]) ~= nil and _global.options["spause"] > 0 then
			_global.utils.subprocess({
				["cancellable"] = false,
				["args"] = { [1] = "sleep", [2] = _global.options["spause"] },
			})
		end
		...
		elseif vars.foundDisp == true then -- We found the display name.
			...
			if vars.foundRes == true then -- We found a matching screen resolution.
				...
				elseif vars.count == 3 then
					if
						(_global.options["interlaced"] == false and vars.interlaced == true)
						or (_global.options["mblacklist"] ~= false and _global.options["mblacklist"][vars.mode] ~= nil)
					then
						-- ignore these modes
					else
		...
		function revertDrr()
			if _global.options["display"] ~= "auto" then
				if _global.options["exitmode"] == "auto" and _global.temp["origmode"] ~= nil then
					os.execute(
						"xrandr --output "
							.. _global.options["display"]
							.. " --mode "
							.. _global.temp["origmode"]
							.. " &"
					)
				else
					os.execute(
